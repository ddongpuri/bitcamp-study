...
public class BoardHandler {
  ...
  public void list(Map<String,String> paramMap, PrintWriter out) throws Exception {
    ...
    for (Board board : boards) {
      ...
      out.printf("<td><a href='detail?no=%d'>%s</a></td>",board.no, board.title); // 제목을 클릭하면 -> 서버에 요청을 보낸다. 
  ...
  public void form(Map<String, String> paramMap, PrintWriter out) {
    ...
    out.println("<table border='1'>");
    out.println("  <tr>");
    out.println("    <th>제목</th><td><input name='title' type='text'></td>"); 
    out.println("  </tr>");
    out.println("  <tr>");
    out.println("    <th>내용</th><td><textarea name='content' rows='10' cols='60'></textarea></td>"); // textarea는 시작태그 끝태그 사이에 있는 것을 값으로 인식( 공백이더라도 ) 
    out.println("  </tr>");
    out.println("  <tr>");
