...
public class Server0110 {
  public static void main(String[] args) throws Exception {
    ...
    // 스캐너에 nextLine()하면, 사용자가 한 줄 입력하기 전까지 hold 되니까.
...
//     25(SMTP), 53(DNS), 80(HTTP)-웹포트 , 110(POP3), 143(IMAP), HTTPS(443)-서버와 클라이언트가 주고받는 데이터를 암호화를 통해 보호 , SSL-telnet(992):보안 텔넷 등
// 
...
//
// => 49152 ~ 65535 (dynamic port) -> 서버 포트로 이 범위의 포트를 사용하면 안된다. 
// - 통신을 하는 프로그램은 반드시 포트번호를 가져야 한다.
//   그래야 OS가 해당 프로그램을 구분할 수 있다.
// - 따라서 클라이언트 프로그램도 포트번호를 갖는데,
//   프로그램에서 결정하는 것이 아니라, 
//   OS로부터 자동 발급 받는다. 
// - 이 범위의 포트번호는 클라이언트가 OS로부터 자동 발급 받는 포트 번호이다. (OS가 동적으로 부여)
