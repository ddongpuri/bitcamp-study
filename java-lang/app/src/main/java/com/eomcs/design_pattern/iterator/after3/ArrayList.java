...
public class ArrayList<E> {
  ...
  public Iterator<E> iterator() {
    // 생성자를 호출 할 때 바깥 클래스의 인스턴스 주소를 넘겨주지 않아도  
    // 컴파일러가 넘겨주는 코드로 자동 변환한다. 
    // 
    return new ArrayListIterator<E>();

    // 위의 코드는 컴파일 될 때 다음과 같이 바뀐다.  
    // return new ArrayListIteratorr<E>(this);
  }

  // non-static nested class(논스태틱 중첩 클래스)
  // => 바깥 클래스의 인스턴스를 사용하는 경우 논클래스 중첩 클래스로 선언하라! 
  // => 왜? 바깥 클래스의 인스턴스 주소를 담을 수 있는  
  class ArrayListIterator<T> implements Iterator<T> {

    // non-static 중첩 클래스인 경우 바깥 클래스의 인스턴스 주소를 받을 필드가 자동 추가된다. 
    // ArrayList<T> list;
    int index = 0;

    // non-static 중첩 클래스인 경우 바깥 클래스의 인스턴스 주소를 받는 파라미터가
    // 생성자가 자동으로 추가된다. 
    // 개발자가 직접 추가할 필요가 없다. 
    //
    ...
    public T next() {
      ...
      // 따라서 ArrayListIterator 내부에서 사용하는 T가 곧 ArrayList의 E와 같다. 
      // 결론 ! 다음과 깉이 형변환 할 수 있다.
